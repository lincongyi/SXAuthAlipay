...
const keypair = {
  privateKey: 'AMPADS+BSbpTPdgX+/yduAxH4ys5lONk4zPgB/NYSlAZ',
  publicKey:
...
const publicKeyPlatform = base64ToHex(
  'BF1bKDid/tOtJP71NlkwkvszkfdPkjpNGj2Z35IVM14qijMyuvlp3ohCAqWZ0J6h7UeAKRe/scFGhHosqfzzCng='
) // 平台公钥-用于sm2加密sm4key
 ...
 */
export const sm4Encrypt = (data: string, sm4Key: string) =>
  sm4.encrypt(data, sm4Key)
 ...
 */
export const sm4Decrypt = (data: string, sm4Key: string) =>
  sm4.decrypt(data, sm4Key)
 ...
 */
export const sm2Encrypt = (sm4Key: string) =>
  `04${sm2.doEncrypt(sm4Key, publicKeyPlatform, cipherMode)}`
 ...
 */
export const sm2Decrypt = (encryptData: string) =>
 ...
 */
export const sm2Sign = (data: string) =>
  sm2.doSignature(data, privateKey, { hash: true, publicKey, der: true })
...
export const sm2VerifySign = (data: string, sigValueHex: string) =>
  sm2.doVerifySignature(data, sigValueHex, publicKey, {
    hash: true,
    publicKey,
    der: true,
  })
